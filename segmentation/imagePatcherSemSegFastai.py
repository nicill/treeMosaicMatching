...
def main():
    ...
    layerNames=["river","decidious","uncovered","evergreen","manmade","Void"]
    layerColors=[0,1,2,3,4,5]
    ...
    for i in range(0,numStepsX):
        ...
        for j in range(0,numStepsY):
            ...
            segmPatch=np.zeros((patch_size,patch_size),dtype=np.uint8)
            segmPatch.fill(5)
            ...
            if(not layerString=="") :
                outputImageName=outputDir+"/"+outputPrefix+"SegmentationPatch"+str(count)+".png"
                if (segmPatch<0).sum()>0 or (segmPatch>5).sum()>0:
                    print("WTF")
                    sys.exit(-1)
                cv2.imwrite(outputImageName, segmPatch )
                #to write the real patch uncomment the following two lines
                outputImageName2=outputDir+"/"+outputPrefix+"SegmentationPatch"+str(count)+"Real.png"
                cv2.imwrite(outputImageName2, imagePatch(image,i*patch_size,j*patch_size,patch_size) )
                #Fprint("for patch "+str(count)+" string is "+layerString)
