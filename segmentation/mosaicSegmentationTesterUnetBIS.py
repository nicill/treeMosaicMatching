...
def trainUnet(validationFile,mosaicPrefix,unetDir,compute=False):
    ...
    lr=5e-3
    modelFileName=mosaicPrefix+"Unet"
    if compute:
...
def main(argv):
    ...
    for i in range(1,numMosaics+1):
        ...
        learn=trainUnet(validFileNameList[i-1],mosaicName,dataDir,compute)
        ...
        for l in codes:
            ...
            if l not in ["Void"]:
                ...
                if manualLayer is not None:
                    manualLayer=np.invert(manualLayerLay)
